...
## Для кого он и в каком порядке ведёт работу

Этот конвейер построен продуктовым дизайнером, и порядок в нём такой, каким работа над продуктом начинается на самом деле: сначала **идея**, потом **дизайн**, потом **контракт поведения** — состояния, тексты и правила, а не классы и сигнатуры, — и только затем план, код и ревью. В центре стоит тот, кто знает, что продукт должен делать, а не тот, кто умеет это набрать.

Из этого выбора следует почти всё остальное устройство:

- **Единственная остановка — контракт, а не код.** Вы подписываете описание поведения, которое можно прочитать целиком и с которым можно спорить. Утверждать дифф никто не просит.
- **На гейте решений спрашивают только продуктовое.** Развилка считается вопросом, если её не решают код, документация и конвенции и если неверный ответ не поймает ни один тест: формулировки, пороги, что считать пустотой, применяется ли контрол мгновенно. Техническую форму — где живёт хук, какой модуль оркестрирует — конвейер решает сам и записывает, а не выносит вам.
- **Дизайн — полноправный источник.** Отдельный разведчик доказывает, что каждая нода макета открывается, перечисляет и те кадры, о которых не спрашивали, и цитирует тексты дословно: макет, который никто не открыл, отправляет исполнителя выдумывать свой.
- **Инженерная строгость не отменена, она делегирована.** Тесты, ассерты которых обязаны падать на сломанной реализации; гейты, которые обязаны быть зелёными; ревьюер с чистым контекстом; финальный проход по всему диффу. То, что сеньор проверяет чтением, здесь проверяется устройством.

Знать свой продукт, своих пользователей и конвенции своего репозитория по-прежнему нужно. Уметь написать фабрику моков — нет.
...
Вынесено из конвейера, который до превращения в плагин уже работал на настоящих фичах, и сверено с сопоставимыми многоагентными решениями и нынешним поколением инструментов спек-ориентированной разработки. Механика и так была на уровне; вынесение сделало явным то, что у большинства остаётся неявным, — шов между конвейером и проектом, на котором он работает. Всё проектное вытеснено в файл профиля, поэтому агенты остаются универсальными и ничто не протухает молча в следующем репозитории.
